...
export const getClubDetail = async (slug) => {
  ...
  try {
    ...
    if (club) {
      const data = await apiRequest(`/clubs/${club.id || club._id}`)
      return data
    }
    return null
  } catch (err) {
...
export const getMessagesBetweenUsers = async (userId, otherId) => {
  const data = await apiRequest(`/messages/${userId}/${otherId}`)
...
export const markMessagesAsRead = async (userId, otherId) => {
  await apiRequest(`/messages/read/${userId}/${otherId}`, { method: "PATCH" })
}
